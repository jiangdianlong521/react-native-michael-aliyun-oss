...
## [README of English](https://github.com/aliyun/react-native-michael-aliyun-oss/blob/master/README.md)
...
注意: react-native版本建议0.44.0及以上，建议使用fackebook官方[react-native-cli](https://www.npmjs.com/package/react-native-cli)或者[create-react-native-app](https://www.npmjs.com/package/create-react-native-app)脚手架进行react native项目构建。react-native-michael-aliyun-oss可通过npm或者yarn安装

* npm

```script
npm install react-native-michael-aliyun-oss  --save
```

* yarn

```script
yarn install react-native-michael-aliyun-oss --save
```
### 自动安装
...
```script
react-native link react-native-michael-aliyun-oss
```
...
- **CocoaPods**
```
pod 'react-native-michael-aliyun-oss', :path => '../node_modules/react-native-michael-aliyun-oss'
````

- **非CocoaPods**

1. 在XCode  Project navigator面板中, 右键单击工程Libraries文件 ➜ 选择`Add Files to <...>` 进入 `node_modules` ➜ `react-native-michael-aliyun-oss` ➜ `ios` ➜ select `RNAliyunOSS.xcodeproj`
...
1. `settings.gradle`
    ```gradle
    include ':react-native-michael-aliyun-oss'
    project(':react-native-michael-aliyun-oss').projectDir = new File(rootProject.projectDir, '../node_modules/react-native-michael-aliyun-oss/android')
    ```
2. `build.gradle`
    ```gradle
    dependencies {
        compile project(':react-native-michael-aliyun-oss')
    }
    ```
...
* step-1:导入AliyunOSS
```
import AliyunOSS from 'react-native-michael-aliyun-oss'
```
* step-2:开启调试模式 (可选)
...
```
1. git clone https://github.com/aliyun/react-native-michael-aliyun-oss.git
2. cd Example
3. npm install
```
